...
def test_update_persisted_tool_is_reoffered(tmp_path):
    skills = SkillStore(tmp_path / "skills")
    skills.persist(ToolSpec(name="adder", description="adds",
                            code='def run(input):\n    return {"v": 1}',
                            inputSchema={"type": "object"}))
    sandbox = ExecutionSandbox(tmp_path / "ws", timeout_sec=5, max_output_bytes=4096)
    deps = RunnerDeps(
        ...
        registry=ToolRegistry(), ask=lambda *a: "y", log_dir=tmp_path,
    )
    runner = AgentRunner(
        deps,
        generated=GeneratedToolManager(tmp_path / "session", sandbox),
...
def _write_runner(tmp_path, path, ask):
    ...
    deps = RunnerDeps(
        ...
        registry=reg, ask=lambda *a: ask, log_dir=tmp_path,
    )
