...
이 에이전트의 ReAct는 내부 reasoning을 별도 필드로 노출하지 않고, LLM이 매 턴 하나의 JSON action을 반환하는 방식으로 표현한다. 런타임은 action을 실행한 뒤 도구 결과, 오류,
검증 결과를 observation으로 컨텍스트에 더한다.
...
[컨텍스트 조립]  system 규칙 + ToolDigest 목록(이름·설명·compact input field hint) + 압축된 과거 + 최근 메시지
   ...
   ▼
┌───────────────────────── 루프 (최대 maxIterations) ─────────────────────────┐
│  [Plan]  LLM 호출 → AgentAction(JSON) 반환                                     │
│        │                                                                    │
│        ▼                                                                    │
...
```python
def dispatch(action, context) -> Result:
    match action.action:
        case "respond":      return respond_flow(action, context)
        case "ask_user":     return ask_user_flow(action, context)   # 5절
        case "call_tool":    return call_tool_flow(action, context)  # 4절
        case "create_tool":  return create_tool_flow(action, context) # 6절
        case "update_tool":  return update_tool_flow(action, context) # 7절
...
[레지스트리 조회]  없으면 → observation "그런 도구 없음. 목록 재확인하거나 생성하세요"
   │
   ▼
[입력 검증]  레지스트리가 inputSchema의 required/properties를 확인 (실패 → observation에 오류)
   │
   ▼
...
[실행]
   ...
   │
   ▼
[출력 정규화]  ToolResult로 성공/실패를 감싸고, 로그 preview는 크기를 제한해 저장
   │
   ▼
[검증]  8절. 결과 상태가 의도를 만족하는지 확인
   │
   ├─ 통과 → observation(결과 요약)
   └─ 실패 → 실패 fallback (4.1)
```
...
```python
def ask_user_flow(action, context):
...
[중복 확인]  같은 목적 도구가 이미 있으면 재사용 안내 (재생성 대신 call_tool 유도)
   │
   ▼
[ToolSpec 검증]  pydantic 모델이 name 규칙, description, code, inputSchema 형태를 검증
   │
   ▼
...
update_tool(name, code, reason)
   │
   ▼
[대상 확인]  현재 runner에 등록된 생성 도구인지 확인. 내장 도구나 등록되지 않은 이름은 수정하지 않는다
   │
   ▼
[코드 교체]  세션 도구의 tool.py를 교체하고 레지스트리에 다시 등록
   │
   ▼
...
- 도구 연속 실패 상한, 파싱 실패 상한, 같은 action 무진전 반복 도달
- 프로토콜 복구 반복 실패

정상 완료 후 세션 중 만든 생성 도구가 있으면 영속화를 제안한다. 중단으로 끝나면 세션 도구
목록을 비우고, 사용자에게는 마지막 의미 있는 observation을 바탕으로 요약을 만든다.
