...
- 권한 판단은 모델이 아니라 런타임으로 강제한다. 쓰기는 묻고, 작업 영역 밖은 거부한다.
...
- 긴 작업을 미리 여러 단계로 쪼개고, 정해 둔 순서대로 실행하는 구조는 아직 없다. 지금은 매 턴
  모델이 다음 action을 고르는 ReAct 루프로 구현되어 있다. 단계가 많은 작업에서는 모델이 중간에 다른 길로
  새기 쉽고, 복구 루프에 의존하게 된다. 모델 성능에 따라 복구 루프 턴 횟수를 초과하고 중단되기도 하며
  그래서 복구 루프에서 이전 이슈를 보다 강하게 고려할 수 있도록 보완이 필요하다.
