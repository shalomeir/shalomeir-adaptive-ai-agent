...
| `observability.py` | 세션별 JSONL trace/span 로그, Rich 출력, bounded LLM 응답 preview, 이벤트를 익스포터로 전달 |
...
집계를 수행한다. 동일한 성공 tool call은 캐시해 재실행과 반복 권한 확인을 피하고, 같은 호출이
계속 반복되면 캐시된 결과를 최종 답변으로 접는다.

약한 모델이 파일 구조를 사용자에게 묻거나, `runPython` 안에서 직접 파일을 쓰거나, 저장된 도구를
읽기 전용 작업에 잘못 재사용하는 경우는 런타임 observation과 정책 검증으로 막는다. JSON dict에
`root.children` 같은 중첩 트리 구조가 보이면 파일 힌트로 재귀 순회를 유도하고, 후속 요청은 대화
컨텍스트와 이전 소스 파일을 활용하도록 planner prompt에서 안내한다. 일반 object tree에서
`type`과 numeric `props` 조건이 명확한 제거·평균 요청은 런타임 fallback으로 처리해 약한 모델의
구조 질문 반복을 끊는다. 반복·파싱 실패처럼 완료 신호 없이 끝난 경우에도 내부 observation을
그대로 사용자에게 내보내지 않고, 사용자용 종료 메시지로 정리한다. CSV 결과는 요청 의도에 맞게
독립 검증하며, 완전 중복 제거와 date 정렬이 틀리면 표준 라이브러리 기반 일반 도구로 교정하고
그 도구를 skill로 저장할 수 있다.
