...
| `source_contracts.py` | 요청·payload의 workspace 경로와 inline structured data 판별 |
...
- D6 객체 트리 조회·변환 도구 실행과 문서 근거 조회
- D7 작업 영역 밖 쓰기 거부
- D8 다중 턴과 권한 확인을 거친 쓰기
- D9 외부 패키지 사용·설치 질문 차단 후 표준 라이브러리 재시도

실제 CLI는 특정 데모 전용 내장 도구로 우회하지 않고, 생성 도구와 저장된 도구의 생성·수정·호출
루프로 파일 처리와 집계를 수행하는 쪽을 기본값으로 둔다. `runPython`은 간단한 산술, regex 확인,
작은 list/filter 검증처럼 짧은 임시 snippet에는 쓸 수 있지만, workspace 파일을 읽어 처리하거나
재사용 가능한 필터링·변환·집계·평균 계산을 해야 하면 생성 도구를 만들거나 재사용한다. 파일 저장은 직접 쓰기
...
managed tool 선호 여부는 필드명이나 일반 응답 동사보다 평균·합계·정렬·필터·변환·저장 같은 작업
동작을 중심으로 판단한다. `hp`, `date`, `type` 같은 필드명이 있다는 이유만으로 generated tool을
강제하지 않는다.
...
저장 등)가 현재 요청과 충분히 겹치지 않으면 이전 도구 호출을 observation으로 되돌린다. 이 판단은
특정 예제 필드명을 whitelist로 박지 않고, 요청과 도구 설명에서 추출한 동작 intent 비율과
파일명만이 아닌 내용 토큰 overlap을 함께 본다. read-only
계산 요청에서 generated tool이 파일 경로만 반환하면 완료로 보지 않고, 요청한 값을 반환하도록 새
도구 생성이나 수정을 유도한다.
파일 출력 결과는 러너가 작업 영역 파일을 다시 읽어 검증한다. 도구가 반환한 출력 경로나
`writeFile` payload를 기준으로 JSON 파싱 가능 여부, CSV header/행 존재 여부, 텍스트 파일의
비어 있지 않음을 확인한다. 요청에 출력 파일명이 명시되어 있으면 도구가 반환한 출력 경로도 그
파일명과 일치해야 한다.
러너는 CSV 중복 제거·정렬·group sum, JSON/object-tree 삭제 조건, markdown table 값 검산 같은
semantic contract를 독립 재계산하지 않는다. 이 판단까지 런타임에 넣으면 특정 표현과 데모 구조에
묶인 예외 로직이 되고, "제외하고 평균을 알려줘" 같은 읽기 전용 요청을 파일 상태 변경으로 오해할
수 있다. semantic 판단은 생성 도구와 LLM에 맡기고, 런타임은 경로·권한·파일 형식·반복 진행 상태처럼
작업 종류와 독립적인 경계만 검증한다. 생성 도구 결과가 파일 출력 sanity check를 통과하면 현재
요청은 검증된 도구 결과로 완료된 것으로 보고 루프를 닫을 수 있다. 성공한 산출물을 다시 LLM에게
넘겨 불필요한 후속 `writeFile`이나 이전 작업 반복으로 오염시키지 않기 위해서다.
...
후속 요청이 이전 tool result의 record list를 변환하는 경우에는 그 list를 generated tool input으로
넘기는 흐름을 허용한다. 현재 요청에 source 파일이 명시된 경우의 임의 inline sample payload와
구분하며, 출력 파일명만 언급된 후속 요청은 그 출력 경로를 source 파일로 오해하지 않는다.
모델이 `respond(final:true)`나 `ask_user`로 사용자 요청을 그대로 되풀이하는 경우도 완료나
질문으로 인정하지 않고, 파일을 직접 열어 실제 계산·변환을 수행하라는 observation을 넣어
다시 계획하게 한다. 파일 경로와 작업 기준이 이미 있는 요청을 `ask_user`로 다시 확인하는 것도
무진전으로 보고, workspace에서 읽을 수 없는 정보가 있을 때만 사용자 질문으로 넘긴다. 파일 결과가 필요한 턴에서 생성 도구를 만들거나 수정했다면, 그 뒤 성공한
...
`writeFile` 호출의 observation 루프로 처리하며, `runPython`은 짧은 임시 snippet에만 쓴다.
현재 요청과 맞지 않는 generated tool 호출은 이전 턴의 도구 실행을 이어가지 않도록
observation으로 차단하고, 차단된 tool action은 `[runtime-state]`의 blocked action 상태에 남겨
같은 후보를 반복 선택하지 않게 한다. 현재 턴에 도구 실행 근거가 없는데 결과형 최종 답변이 이전 assistant
...
차단해, 임의 데이터가 실제 파일 처리로 둔갑하지 않게 한다. 반대로 사용자 요청 자체에 inline JSON/CSV
데이터가 들어 있고 workspace 파일 경로가 없으면, 모델이 `data.json` 같은 임의 파일명을 만들어
호출하거나 파일을 달라고 묻는 흐름을 source contract 위반으로 되돌린다. 이 판별은 특정 데모 이름이
아니라 요청 본문과 payload의 경로·구조를 비교하는 `source_contracts.py`의 순수 helper에 둔다.
