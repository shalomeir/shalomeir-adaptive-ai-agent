...
루프로 파일 처리와 집계를 수행하는 쪽을 기본값으로 둔다. `runPython`은 작은 계산, 빠른 진단,
스키마 확인, 검증처럼 관리형 도구로 남길 가치가 낮은 보조 실행에 한정한다. 파일 저장은 직접 쓰기
우회를 감지해 권한 정책 경계로 올리고, 생성 도구가 `output`, `dst` 같은 출력 경로 입력을 받으면
작업 영역 밖 경로만 실행 전에 거부한다. 동일한 성공 tool call은 캐시해 재실행과 반복 권한 확인을
피하고, 같은 호출이 계속 반복되면 캐시된 결과를 최종 답변으로 접는다.
...
생성 도구와 `writeFile` 호출의 observation 루프로 처리하며, `runPython`은 보조 수단으로만 둔다.
