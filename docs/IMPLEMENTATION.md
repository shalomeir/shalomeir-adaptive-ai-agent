...
읽기 전용 작업에 잘못 재사용하는 경우는 런타임 observation과 정책 검증으로 막는다. CSV 결과는
요청 의도에 맞게 독립 검증하며, 완전 중복 제거와 date 정렬이 틀리면 표준 라이브러리 기반 일반
도구로 교정하고 그 도구를 skill로 저장할 수 있다. 다중 턴 표 저장처럼 이전 결과를 참조하는
명확한 작업은 대화 컨텍스트와 원본 파일을 함께 확인해 한 번의 쓰기 승인으로 처리한다.
