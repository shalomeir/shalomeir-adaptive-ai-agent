...
실제 CLI에서 D1처럼 명확한 JSON 숫자 필드 이름·평균 질의는 `queryJsonNumeric`, D3처럼 명확한 CSV
읽기 전용 그룹 합계는 `aggregateCsv`, D2처럼 명확한 CSV 중복 제거·date 정렬·저장 요청은
`normalizeCsv` 내장 도구로 직접 처리한다. 약한 로컬 모델이 잘못된 검증 스크립트를 반복해도
이 경로에서는 LLM 루프에 들어가지 않아 안정적으로 끝난다. 생성 도구 기반 D1/D2/D3/D5 경로는
통합 테스트에 남겨 재사용 시나리오를 계속 검증한다.
