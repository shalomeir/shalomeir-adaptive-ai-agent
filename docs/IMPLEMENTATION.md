...
| `tools/builtins.py` | 내장 도구: 파일 읽기·쓰기·목록, JSON hp 질의, CSV 집계·정리, Python 실행, 문서 조회, 사용자 질문 |
...
8. **LLM 클라이언트와 검증** — OpenAI 호환 클라이언트, 결정적 테스트용 fake, 검증 helper.
   클라이언트는 JSON object 응답 모드를 우선 요청하고, 미지원 호환 서버에서는 같은 요청을
   옵션 없이 재시도한다. 이어서 트레이서가 같은 이벤트를 익스포터로도 전달하도록 보강해,
   나중에 외부 모니터링을 붙일 자리를 열어 두었다.
...
실제 CLI는 특정 데모 전용 내장 도구로 우회하지 않고, `runPython` 또는 생성 도구로 파일 처리와
집계를 수행한다. `runPython`은 읽기·계산용으로 쓰고 파일 저장은 `writeFile`을 거치게 해
권한 정책이 항상 같은 경계에서 적용되도록 했다. 생성 도구가 `output`, `dst` 같은 출력 경로
입력을 받으면 작업 영역 밖 경로만 실행 전에 거부하고, 작업 영역 내부 상대 경로는 임시/출력
파일로 사용할 수 있게 둔다. 동일한 성공 tool call은 캐시해 재실행과 반복 권한 확인을 피하고,
같은 호출이 계속 반복되면 캐시된 결과를 최종 답변으로 접는다.
또한 CSV 정리, object tree 변형, 이전 필터 결과의 markdown 표 저장처럼 데모에서 반복적으로
깨지는 결과 상태는 러너가 작업 영역 파일을 다시 읽어 검증한다. 검증 실패는 모델에게 구체적인
expected order나 남은 node id를 observation으로 되먹이고, 검증 성공은 추가 LLM final 없이도
완료 summary로 종료할 수 있다.

약한 모델이 파일 구조를 사용자에게 묻는 경우는 런타임 observation으로 되먹여 다시 계획하게
한다. JSON dict에 `root.children` 같은 중첩 트리 구조가 보이면 파일 힌트로 재귀 순회를 유도하고,
후속 요청은 대화 컨텍스트와 이전 소스 파일을 활용하도록 planner prompt에서 안내한다. 반복·파싱
실패처럼 완료 신호 없이 끝난 경우에도 내부 observation을 그대로 사용자에게 내보내지 않고,
사용자용 종료 메시지로 정리한다.
...
`call_tool`로 실행하라는 구체적인 payload 힌트를 observation에 넣는다.
`respond`의 `final` 필드는 생략하면 최종 응답으로 처리하고, 중간 상태 메시지만 보낼 때
`final:false`를 명시하게 했다. 약한 모델이 짧은 대화 응답에서 `final`을 자주 생략하기 때문이다.
CSV 정리, object tree 변형, markdown 표 생성 같은 도메인 작업은 러너가 직접 수행하지 않고
`runPython`, 생성 도구, `writeFile` 호출의 observation 루프로 처리한다.
