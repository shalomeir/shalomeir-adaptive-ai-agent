...
| `sandbox.py` | subprocess 격리 실행(타임아웃, 작업 디렉터리 제한, 출력 제한) |
| `policy.py` | ALLOW / DENY / ASK_USER 권한 게이트 |
| `tools/base.py`, `tools/registry.py` | 도구 추상화와 레지스트리(이름·설명 digest 노출) |
| `tools/builtins.py` | 내장 도구: 파일 읽기·쓰기·목록, Python 실행, 문서 조회, 사용자 질문 |
...
러너 의존성으로 넘겨 배선을 닫고 README의 두 표현(네트워크 차단은 미강제, 도구 코드는 디스크
보관)을 정확히 고쳤다.
...
- 생성 도구 품질은 모델과 검증 루프에 의존한다. 스모크 결과는 참고용이고, 실제 호출 오류가
  수정을 이끈다.
- 현재 환경 변수로 바꿀 수 있는 설정은 일부이며 나머지는 코드 기본값이다.
