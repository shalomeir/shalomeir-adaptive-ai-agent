...
약한 모델이 파일 구조를 사용자에게 묻거나 저장된 변환 도구를 읽기 전용 작업에 잘못 재사용하는
경우는 런타임 observation으로 되먹여 다시 계획하게 한다. JSON dict에 `root.children` 같은
중첩 트리 구조가 보이면 파일 힌트로 재귀 순회를 유도하고, 후속 요청은 대화 컨텍스트와 이전
소스 파일을 활용하도록 planner prompt에서 안내한다. 반복·파싱 실패처럼 완료 신호 없이 끝난
경우에도 내부 observation을 그대로 사용자에게 내보내지 않고, 사용자용 종료 메시지로 정리한다.
