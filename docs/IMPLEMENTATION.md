...
| `tools/builtins.py` | 내장 도구: 파일 읽기·쓰기·목록, JSON hp 질의, CSV 정리, Python 실행, 문서 조회, 사용자 질문 |
...
실제 CLI에서 D1처럼 명확한 JSON hp 이름·평균 질의는 `queryMonsterHp`, D2처럼 명확한 CSV
중복 제거·date 정렬·저장 요청은 `normalizeCsv` 내장 도구로 직접 처리한다. 약한 로컬 모델이
잘못된 검증 스크립트를 반복해도 이 경로에서는 LLM 루프에 들어가지 않아 안정적으로 끝난다.
생성 도구 기반 D1/D2/D5 경로는 통합 테스트에 남겨 재사용 시나리오를 계속 검증한다.
